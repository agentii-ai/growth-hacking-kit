...
# This script extracts the version from the git tag
# For tag-based releases, we use the tag itself as the version

# Check if we're in a GitHub Actions environment with a tag
if [[ -n "${GITHUB_REF:-}" ]] && [[ "$GITHUB_REF" =~ refs/tags/(v[0-9]+\.[0-9]+\.[0-9]+) ]]; then
  VERSION="${BASH_REMATCH[1]}"
  echo "Extracted version from tag: $VERSION" >&2
  echo "$VERSION"
  exit 0
fi
